...
- get_budget_status — returns the current mission budget state, remaining headroom for each metric (cost, total tokens, wall-clock, agent runs), per-metric used fraction, and a coarse risk level (ok | warning | critical | exhausted). Visibility only; does not reserve or refuse budget. Use this to decide whether to start a costly step.
- reload_budget — re-reads the budget configuration from ratel.json and updates the in-memory budget limits. Call this AFTER the user has increased the budget in ratel.json. Clears the exhausted flag if the new limits accommodate current usage; re-exhausts if usage still exceeds the new limits. If the budget is exhausted, ask the user to increase it in ratel.json, then call reload_budget() to apply the new limits before retrying execution tools.`;
